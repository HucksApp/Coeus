...
if __name__ == '__main__':
    ...
    if training:
        ...
        model = CoeusClassification(training=True, dataset_path=data_dir, save_dir=save_dir, title="toyota_models")
        model.to(model.device)
        model.train_in_progressive(epochs_per_run=2)
    else:
        model = CoeusClassification(training=False, dataset_path=data_dir, save_dir=save_dir, title="toyota_models")
